...
using Time = MusicXml.Domain.Time;
	...
	public static class MusicXmlParser
	{
		...
		private static Pitch GetPitch(XmlNode noteNode)
		{
			...
			if (pitchNode != null)
			{
				...
				if (stepNode != null)
					pitch.Step = stepNode.InnerText[0].ToString();
		...
		private static int GetSpeed(XmlNode directionNode)
		{
			
			var lineNode = directionNode.SelectSingleNode("direction-type");
			if (lineNode != null)
			{
				var speedNode = lineNode.SelectSingleNode("words");
				if (speedNode != null)
				{
					return Convert.ToInt32(speedNode.InnerText);
				}
			}
			return 120;
		}
		...
		private static XmlDocument GetXmlDocument(string textfile)
		{
			var document = new XmlDocument();

			//var xml = GetFileContents(filename);
			document.XmlResolver = null;
			document.LoadXml(textfile);
